...
import cc.vimc.mcbot.exception.AuthFailureException;
import cc.vimc.mcbot.exception.RconClientException;
...
//        return new String(bodyBytes, PAYLOAD_CHARSET);
    ...
    private static ByteBuffer toByteBuffer(int requestId, int type, String payload) {
        ByteBuffer buffer = ByteBuffer.allocate((3 * Integer.BYTES) + new String(payload.getBytes(UTF_8),PAYLOAD_CHARSET).length() + (2 * Byte.BYTES));
        buffer.order(ByteOrder.LITTLE_ENDIAN);

        buffer.putInt((2 * Integer.BYTES) + new String(payload.getBytes(UTF_8),PAYLOAD_CHARSET).length() + (2 * Byte.BYTES));
        buffer.putInt(requestId);
        buffer.putInt(type);
        buffer.put(payload.getBytes(UTF_8));
        buffer.put((byte) 0);
        buffer.put((byte) 0);
